...
void* handlerThread(void *arg)
{
    ...
    while(isOn)
    {
        ...
        if(keyBoardNum==10) { if(uvcState==1) v4l2_control(0x8000);if(OUTPUTMODE==6){shutterCnt=1;nucFreeze=true;firstShutFrame=true;}}//enter:shutter
        if(keyBoardNum==86) { if(uvcState==1&&OUTPUTMODE==6){ v4l2_control(0x8081);KsendFlag=true;}}//receive K
        if(keyBoardNum==48) { 
        ...
        if(keyBoardNum==73) { if(uvcState==1){palette=7;v4l2_control(0x8800);} }//i lava Rainbow
        if(keyBoardNum==79) { if(uvcState==1){palette=8;v4l2_control(0x8802);} }//o th Iron Rainbow
        ...
        if(keyBoardNum==70) {if(uvcState==1);v4l2_control(0x8021); rangeMode=400;hightemperature_Flag=1;sleep(1); v4l2_control(0x8000); sleep(1);needRecal=1;sleep(10); v4l2_control(0x8000); sleep(1);needRecal=1;}//f change rangeMode 400
        ...
        if(keyBoardNum==74) { int frate=0;float new_float_tmp006= 6.0f;new_float_tmp006=new_float_tmp006*10; if(new_float_tmp006<0) {frate=(int)(256+new_float_tmp006);}else {frate=(int)(new_float_tmp006);} frate=0x9500|frate; v4l2_control(frate);sleep(1); v4l2_control(0x8000); sleep(2);v4l2_control(0x8000); rightData=0; sleep(2); v4l2_control(0x80fe);usleep(100000);rightData=1;needRecal=1;}//j TMP006                    
        ...
        if(keyBoardNum==75) {if(uvcState==1){autoCalibtateShutter_Flag=TRUE;sleep(1);}}
        if(keyBoardNum==76) {
        ...
        if(keyBoardNum==77){if(uvcState==1){temperautreOutput_Flag=TRUE;sleep(1);}}
        if(keyBoardNum==78){if(uvcState==1){distanceFix_flag=TRUE;sleep(1);}}
        if(keyBoardNum==66){if(uvcState==1){saveImage_Flag=TRUE;sleep(1);}}
    }
}
...
void* renderThread(void *arg)//
{
    ...
    {
        if(v4l2_control(0x8082) == FALSE)//控制机芯切换为8004 原始数据输出   Switch to 8004 mode, output raw data 
        {
            printf("control fail~~\n");
            //uvcState=0;
        }
    ...
    while(uvcState)
    {
        for(n_buffers = 0; n_buffers < 4; n_buffers++)
        {
            ...
            if(rightData&&(checkresult==0))
            {
                ...
                // }
                // float kc = 0;
                // int kc_position=amountPixels+11;
                // memcpy(&kc,fourLinePara+kc_position,sizeof( float));
                // //printf ("kc:%f\n", kc);
                // if(IMAGEWIDTH==640&&rangeMode==400&&hightemperature_Flag==1)
                // {
                //     int kc_640_position=amountPixels+255+69+8 ;
                //     memcpy(&kc,fourLinePara+kc_640_position,sizeof( float));
                //     printf ("640 kc:%f\n", kc);
                //     hightemperature_Flag=0;
                // }
                ...
                // char* charx_fourLinePara=(char*)fourLinePara;
                // char TMP006=0;
                // switch (IMAGEWIDTH)
                // {
                //     case 384:
                //     case 640:
                //         memcpy(&TMP006,charx_fourLinePara+2*(3*IMAGEWIDTH+255)+95,sizeof(char));
                //         break;
                //     case 240:    
                //     case 256:
                //         memcpy(&TMP006,charx_fourLinePara+2*2*IMAGEWIDTH+95,sizeof(char));
                //         break;    
                // }
                // float float_TMP006=TMP006/10.0f;
                //printf("float_TMP006:%f\n",float_TMP006);
                //printf("delayy:%d\n",delayy);
                
                if(delayy%1500==10)//每三分钟（对应30)打一次快门，人体高精度版本每分钟打一次   Shutter calibrates once every three minutes, for body temperature measuring, once per minute
                {
                ...
                //if(delayy%25==10)//一般一秒钟查一次表确定对应点的温度，首次查表前要计算完temperatureTable
                ...
                if(temperautreOutput_Flag==TRUE)
                {
                    ...
                    {
                        EAthermometrySearch(IMAGEWIDTH,IMAGEHEIGHT,temperatureTable,orgData,temperatureData,correction,rangeMode,OUTPUTMODE);
                        printf("centerTmp:%.2f,maxTmp:%.2f,minTmp:%.2f\n",temperatureData[0],temperatureData[3],temperatureData[6]); 
                    }
                           
                    temperautreOutput_Flag=FALSE;                    
                }
                else if(OUTPUTMODE==5&&temperautreOutput_Flag==TRUE)
                {
...
int main()
{
    ...
    while(isOn)
    {
    	...
    	//if(temperatureData != NULL){
    		//temperautreOutput_Flag=TRUE;
    		//printf("Max_Temp:%f, Min_Temp:%f, Center_Temp:%f\n",temperatureData[3], temperatureData[6], temperatureData[0]); //在这里temperatureData还没有初始化分配空间
    	//}
        int nNub = KeyTestNew();
        switch (nNub)
        {
            ...
            case 77://m output center temperature
