...
class MeterReader(object):
    ...
    def __init__(self, img):
        ...
        if center is None or radius is None:
            self.line_width = None  # 表盘展开为直线的长度,按照表盘的图像像素周长计算
            self.line_height = None  # 表盘展开为直线的宽度，该设定按照围绕圆心的扇形宽度计算，需要保证包含刻度以及一部分指针
            self.circle_radius = None  # 预设圆盘直径，扫描的最大直径，需要小于图幅，否者可能会报错
            self.circle_center = None  # 圆盘指针的旋转中心，预设的指针旋转中心
            self.threshold = 0.5
        else:
        # Custom parameters
            self.line_width = int(2 * np.pi * radius)  # 表盘展开为直线的长度,按照表盘的图像像素周长计算
            self.line_height = int(radius*0.35)  # 表盘展开为直线的宽度，该设定按照围绕圆心的扇形宽度计算，需要保证包含刻度以及一部分指针
    ...
    def __call__(self, point_mask_resized, dail_mask_resized):
        if self.circle_radius is None or self.circle_center is None:
            return None
        # 可视化掩码
        # plt.subplot(1, 2, 1)
